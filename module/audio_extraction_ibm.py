...
def convert_webm_to_mp3(webm_file: io.BytesIO, mp3_path: str, bitrate: str = '320k'):
    """
    메모리에서 webm 파일을 mp3 형식으로 변환하고 포즈를 분석합니다.
    :param webm_file: 메모리에서의 webm 파일
    :param mp3_path: 변환할 mp3 파일의 경로
    :param bitrate: MP3 비트레이트 (예: '192k', '256k', '320k')
    ...
    #     '-q:a', '0',  # 오디오 품질을 최적화
    #     '-map', 'a',  # 오디오 스트림만 추출
    #     '-b:a', bitrate,  # 비트레이트 설정
    #     mp3_path
    # ]
    command = [
        'ffmpeg', '-i', temp_webm_path, '-y',
        '-ar', '48000',  # 샘플링 레이트를 48kHz로 설정
        '-b:a', bitrate,  # 비트레이트 설정
        mp3_path
    ]
