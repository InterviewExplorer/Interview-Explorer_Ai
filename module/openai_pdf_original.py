...
async def pdf(pdf_path, max_retries=3):
    ...
    "technical_skills": "백엔드:  / 프론트엔드:  / ai:  / 도구:  / db: / 머신러닝: / 언어: / 기타: "
    ...
    Precautions:
        ...
        - Always display the name without any spaces.
        - The date of birth must be displayed using only numbers and '-' (hyphens).
...
Ensure all text is properly formatted and in Korean.
    ...
    """
        # - Extract only the information exactly as shown in the resume. For information not present in the resume, indicate as '없음' (None) or '0'.
        # - For 'technical_skills', if there are no values, don't return the key at all.
        # - Sum up all the work experience mentioned in the resume and express it only in years and months.
        # - Only recognize work experience when the company name is explicitly mentioned.
