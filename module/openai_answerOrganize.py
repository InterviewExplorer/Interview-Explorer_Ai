...
def answerOraganize(answers: str, questions: str, job: str, type: str, followQuestion: str)-> dict:
    ...
    print(f"이거 값 뭐냐1: {followQuestion}")
    print(f"이거 값 뭐냐2: {questions}")
    print(f"이거 값 뭐냐3: {answers}")
    ...
    if followQuestion is None:
        ...
        if type == "technical":
            ...
            # Task
            - Review {questions} with their corresponding {answers}.
            - Identify a question-answer pair that is different from the one that led to the previous follow-up question: {followQuestion}
            - Generate a new follow-up question in Korean for this identified question-answer pair, focusing on technical depth.

            # Policy
            - Carefully analyze the question-answer pairs to find one that hasn't been explored further yet.
            - Create only one follow-up question for the selected question-answer pair.
            - Ensure the new question is for a different topic than the one addressed by {followQuestion}.
            - The follow-up question must be in Korean.
            - The follow-up question should aim to explore related technologies, models, or methodologies.
            - Instead of asking for specific examples, create questions that verify understanding of related technical concepts.
            - When asking the follow-up question, reference the original question and answer. For example, start with a phrase like "Regarding [original question], you answered [brief summary of answer]. ..."
            ...
            - Review {questions} with their corresponding {answers}.
            - Identify a question-answer pair that is different from the one that led to the previous follow-up question: {followQuestion}
            - Generate a new follow-up question in Korean for this identified question-answer pair, focusing on exploring the candidate's experience and competencies.
